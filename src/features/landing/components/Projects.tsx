    ...
    return (
        <Section id='projects' label='Projects'>
            {projects.map(({ slug, title, description, tech }) => (
                ...
                >
                    <div>
                        <p className='text-[15px] font-semibold text-black mb-1.5 group-hover:underline'>{title}</p>
                        <p className='text-[13px] text-gray leading-[1.65] mb-3'>{description}</p>
                        <div className='flex flex-wrap gap-1.5'>
                            {tech.map((t) => (
                                <span key={t} className='text-[11px] font-medium tracking-[0.04em] uppercase text-gray border border-gray/20 px-2 py-[3px]'>
                                    {t}
                                </span>
                            ))}
                        </div>
